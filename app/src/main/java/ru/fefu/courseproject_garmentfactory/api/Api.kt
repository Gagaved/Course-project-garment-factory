...
interface Api {
    ...
    fun clothDecommission(
        @Header("Authorization") token: String,
        @Path("article") article: Int,
        @Path("number") number: Int,
        @Query("length") length: Float
    ): Call<ClothDecommissionResponse>

    @PATCH("api/v1/accessory/{article}")
    fun accessoryDecommission(
        @Header("Authorization") token: String,
        @Path("article") article: Int,
